...
def cadastrar_usuario():
    ...
    if form_cad_user.validate_on_submit():
        ...
        cpf_ja_cadastrado = UsuarioModel.query.filter_by(
            cpf_usuario=form_cad_user.cpf.data).first()
        usuario_ja_cadastrado = UsuarioModel.query.filter_by(
            nome_usuario=form_cad_user.nome_usuario.data).first()
        cpf_valido = validar_cpf(form_cad_user.cpf.data)
        email_ja_cadastrado = UsuarioModel.query.filter_by(
            email=form_cad_user.email.data).first()

        # inicio das verificações
        # verifica se o cpf já não está cadastrado (erro chave 1ª)
                    ...
                    if not email_ja_cadastrado:
                        if form_cad_user.imagem.data.filename != "":
                            nome_arquivo = secure_filename(
                                form_cad_user.imagem.data.filename)
                            form_cad_user.imagem.data.save(os.path.join(
                        ...
                        erros.append(
                            'Email já cadastrado. Por favor tente outro')
                else:
                    # nome de usuário ja foi usado, mostrará um erro
                    erros.append(
                        'Nome de usuário já cadastrado. Por favor tente outro')
            else:
    ...
    if form_edit_user.validate_on_submit():
        if imagem.data.filename != "":
            ...
            if arquivo_atual != "ovo.jpg":
                os.remove(os.path.join(
                    app.config['UPLOAD_FOLDER'], arquivo_atual))
            imagem.data.save(os.path.join(
                app.config['UPLOAD_FOLDER'], nome_arquivo))
        ...
        if usuario.data != current_user.nome_usuario:
            usuario_existente = UsuarioModel.query.filter_by(
                nome_usuario=usuario.data).first()
        else:
        ...
        if usuario_existente is None:
            if email_existente is None:
                ...
                avalicao_do_usuario = AvaliacaoSiteModel.query.filter_by(
                ...
                db.session.commit()
                flash(f'Alterações na conta realizadas com sucesso',
                      category="sucesso")
...
def logar_usuario():
    ...
    if form_login.validate_on_submit():
        usuario = UsuarioModel.query.filter_by(
            nome_usuario=form_login.nome_usuario.data).first()
        if usuario is not None:
            verificacao_senha = usuario.checar_senha(form_login.senha.data)
            if verificacao_senha:
                login_user(usuario)
                flash(
                    f'Você foi logado com sucesso {current_user.nome_completo}', category="sucesso")
                ...
                erros.append(
                    'Usuário e/ou senha inválido. Por favor, digite outro.')
        else:
            erros.append(
                'Usuário e/ou senha inválido. Por favor, digite outro.')
...
def cadastrar_cidade():
    ...
    if form_cad_city.validate_on_submit():
        ...
        while True:
            id_gerado = uuid4()
            busca_por_id_repetido = CidadeModel.query.filter_by(
                id=str(id_gerado)).first()
            if busca_por_id_repetido is None:
                break

        cidade_valida = coletar_objetos_climaticos(
            form_cad_city.nome_cidade.data)
        ...
        if cidade_valida is not None:  # cidade válida
            ...
            if cidade_ja_cadastrada is not None:
                ...
                db.session.commit()
                flash(
                    f'A cidade {nova_cidade.nome_cidade} foi adicionada com sucesso', category="sucesso")
                return redirect('/lista_cidades')

        else:  # cidade não válida
            nome_cidade = form_cad_city.nome_cidade.data
            erros.append(
                f'{nome_cidade} não é um nome de cidade válido para a API de clima. Por favor tente outro.')
...
def editar_cidade(id):
    ...
    form_edicao_city.descricao_cidade.render_kw = {
        "placeholder": cidade.descricao}
    ...
    if form_edicao_city.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Cidade {cidade.nome_cidade} editada com sucesso',
              category="sucesso")
        return redirect('/lista_cidades')
...
def exibir_cidade(id):
    ...
    objeto_cidade, objeto_previsao = coletar_objetos_climaticos(
        cidade.nome_cidade)
...
@login_required
def avaliar_site():
    ja_avaliado = AvaliacaoSiteModel.query.filter_by(
        cpf_usuario=current_user.cpf_usuario).first()
...
def excluir_conta():
    ...
    avaliacao = AvaliacaoSiteModel.query.filter_by(
        cpf_usuario=current_user.cpf_usuario).first()
    ...
    foto_path_iguais = UsuarioModel.query.filter_by(
        caminho_foto=current_user.caminho_foto).all()
...
def cancelar_notificacao(cpf_usuario, id):
    ...
    if id != 'todos':
        ...
        if cidade.notificavel:
            ...
            db.session.commit()
            flash(
                f'Notificação da cidade {cidade.nome_cidade} removida com sucesso', category="sucesso")
            return redirect('/home')
    else:
        ...
        for cidade in cidades:
            if cidade.notificavel:
                ...
                db.session.commit()
        flash(
            f'Todas as notificações de {usuario.nome_usuario} foram removidas com sucesso', category="sucesso")
        return redirect('/home')
...
def solicitar_recuperacao_senha():
    ...
    if form.validate_on_submit():
        ...
        usuario = UsuarioModel.query.filter_by(
            cpf_usuario=cpf_padronizado).first()
        if usuario is not None:
...
def recuperar_senha(cpf):
    ...
    if usuario is not None:
        ...
        db.session.commit()
        mensagem = Message('Get Weather - Recuperar senha',
        ...
        flash(f'Erro ao enviar e-mail de recuperação: CPF informado inválido.',
              category="error")
...
def confirmar_email(cpf, token):
    try:
        ...
        db.session.merge(usuario)
        db.session.commit()
        flash(f'E-mail confirmado com sucesso. Você já pode usar nossos serviços',
              category="sucesso")
        return redirect('/home')
...
def enviar_email_confirmacao(email):
    ...
    if usuario is not None:
        ...
        mensagem = Message('Get Weather - Confirmar E-mail',
                           recipients=[email])
        link = url_for('confirmar_email', cpf=usuario.cpf_usuario,
        ...
        flash(f'E-mail inválido para enviar a confirmação de E-mail',
              category="sucesso")
    return redirect('/home')
    ...
    if cpf == "todos":  # mandar email para todos os usuários
        ...
        for usuario in usuarios:
            if usuario.email_confirmado:
                cidade_notificavel = CidadeModel.query.filter_by(
                    cpf_usuario=usuario.cpf_usuario, notificavel=True).first()
                if cidade_notificavel is not None:
                    ...
                    try:
                        ...
                        flash(
                            f'Erro ao enviar e-mail para {usuario.nome_usuario}: Nenhuma cidade dele(a) é notificável.', category="erro")
            else:
                flash(
                    f'Erro ao enviar e-mail para {usuario.nome_usuario}: e-mail não confirmado.', category="erro")
        else:
        ...
        if usuario is not None:  # para isso, o cpf precisa estar cadastrado
            if usuario.email_confirmado:
                cidade_notificavel = CidadeModel.query.filter_by(
                    cpf_usuario=cpf, notificavel=True).first()
                ...
                flash(
                    f'O e-mail {usuario.email} não foi confirmado. Por favor, confirme seu e-mail acessando seu perfil', category="erro")
        else:
...
def coletar_objetos_climaticos(nome_cidade: str):
    try:
        ...
        objeto_previsao = api_clima.three_hours_forecast(
